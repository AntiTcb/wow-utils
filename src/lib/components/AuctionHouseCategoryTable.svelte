<script>
    import {
        DataTable, Pagination, Toolbar, ToolbarContent,
        ToolbarSearch
    ...
    let comparisonRealm = 'atiesh'
    let comparisonFaction = 'alliance';
    ...
    const getServerItemPrice = async (realm, faction, itemUniqueName) => {
        const res = await fetch(`https://api.nexushub.co/wow-classic/v1/items/${realm}-${faction}/${itemUniqueName}`, {
...
<details data-category={category} data-realm={realm} data-faction={faction} style="display: {display}" {...$$restProps}>
    <summary class="is-size-4" on:click={() => lazyLoaded = true}>{category} {items.length ? `(${items.length})` : ""}</summary>
    {#if lazyLoaded }
        <DataTable
            sortable
            expandable
            ...
            <svelte:fragment slot="cell" let:row let:cell>
                {#if cell.key === 'itemName'}
                    <div class="is-flex is-align-items-center" style="gap: 0.25rem">
                        <img src="https://wow.zamimg.com/images/wow/icons/large/{row.iconUrl}.jpg" alt="{row.itemName}" height={24} width={24} />
                        <OutboundLink href="https://tbc.wowhead.com/item={row.itemId}">{cell.value}</OutboundLink>
                    </div>
                {:else if cell.key === "server" }
                    <OutboundLink href="https://nexushub.co/wow-classic/items/{realm}-{faction}/{row.uniqueName}" class="is-capitalized">{cell.value}</OutboundLink>
            ...
            <svelte:fragment slot="expanded-row" let:row>
                <div class="add-comparison-container is-inline-flex is-justify-content-end is-align-items-end p-1">
                    <RealmSelect bind:selected={comparisonRealm} serverFilter={((s) => s.slug !== realm)} />
                    <FactionSelect bind:selected={comparisonFaction} />
                    <button class="button is-rounded is-smaller" type="button" on:click={async () => await addServerComparison(comparisonRealm, comparisonFaction, row.uniqueName, row.id)}>
