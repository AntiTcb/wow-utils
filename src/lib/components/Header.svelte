<script>
    import { Header, HeaderNav, HeaderNavItem, HeaderUtilities, HeaderAction, HeaderPanelLinks, HeaderPanelLink, HeaderGlobalAction } from 'carbon-components-svelte';
...
<Header platformName="Wow Utils" href="/">
    ...
    <HeaderNav>
        {#if $defaultRealm && $defaultFaction}
            <HeaderNavItem text="Server Price Comparison" href="/compare/{$defaultRealm}/{$defaultFaction}" on:click={handleLinkClick} />
        {:else}
