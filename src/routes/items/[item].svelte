<script context="module">
    export async function load({fetch, params}) {
        const res = await fetch(`https://api.nexushub.co/wow-classic/v1/item/${params.item}`.toLowerCase());
...
<script>
    ...
    const getServerItemPrice = async (realm, faction) => {
        const res = await fetch(`https://api.nexushub.co/wow-classic/v1/items/${realm}-${faction}/${item.uniqueName}`);
...
<img src={item.icon} alt={item.name}/><h2><a href='https://tbc.wowhead.com/item={item.itemId}' target='_blank'>{item.name}</a></h2>
...
<div class="field">
    ...
    <div class="select">
        <select id='comparison-server-1' name='comparison-server-1'>
            {#each $servers as s}
                <option value={s.slug} selected={(s.slug === "atiesh" ? "selected" : "")}>{titleCase(s.name)}</option>
            {/each}
        </select>
    </div>
    <button class="button is-rounded is-success" type='button' id='addServerBtn' on:click={async () => await addTableRow(document.querySelector('#comparison-server-1').value, document.querySelector('#comparison-faction-1').value)}>Add Comparison Server</button>
</div>
...
<table class="table">
    ...
    <tbody id='item-table-body'>
        {#if defaultRealm !== '' }
            ...
            {:then item}
                <tr>
                    <td><a href="https://nexushub.co/wow-classic/items/{defaultRealm}-{faction}/{item.uniqueName}" target="_blank">{titleCase(defaultRealm)} - {titleCase(faction)}</a></td>
